...
st.subheader("bem vindo(a) você está no.")
st.title("💬 Consultor de relacionamentos")
...
# 🔒 Sistema de trava com senha
if "liberado" not in st.session_state:
    st.session_state.liberado = False

if not st.session_state.liberado:
    senha_acesso = st.text_input("🔑 Digite a senha para liberar o chat:", type="password")
    if st.button("🔓 Liberar"):
        if senha_acesso == "luc123":  # senha de acesso ao chat
            st.session_state.liberado = True
            st.success("✅ Chat liberado para todos!")
        else:
            st.error("Senha incorreta! 🚫")
    st.stop()  # bloqueia o resto do app até liberar
...
# 💲 Botão para exibir preço
st.subheader("💲 Consultoria")
if st.button("Clique aqui para ver o preço"):
    st.info("O preço da consultoria é *R$ 40,00*.")  # <-- você pode mudar esse valor
